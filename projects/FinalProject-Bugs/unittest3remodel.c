...
//////////////////////////////////////////////////
	...
	G.hand[G.whoseTurn][0] = remodel; //mine added to deck 
	G.hand[G.whoseTurn][1] = ambassador; // add estate to deck 
	...
	returnVal = cardEffect( remodel, 1, tribute,  -1, &G, 0, placeholder); 
	
//	printGameState(preG);
//	printGameState(G);

	//Test results for expected outcome
	printf("TEST Bug 3 RESULTS: upgrade a ambassador to a tribute\n");
...
//////////////////////////////////////////////////
	...
	//make testing changes to gamestate
	G.hand[G.whoseTurn][0] = remodel; //mine added to deck 
	G.hand[G.whoseTurn][1] = embargo; // add estate to deck 
	G.hand[G.whoseTurn][2] = estate; 
	G.hand[G.whoseTurn][3] = estate;
	G.hand[G.whoseTurn][4] = estate;
...
//	printGameState(G);
	...
	//Test results for expected outcome
	printf("TEST Bug 3-b RESULTS: upgrade a embargo to a tribute\n");
	assert( -1, returnVal, "Return Value should be -1" );
	assert( 0 , memcmp(&preG, &G, sizeof(G)), "Memory blocks should not have been altered") ;
	return 0;
}
